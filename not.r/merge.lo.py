...
def loadSpecies(s,res):
	...
	for l in gtf:
		...
		if k in res:
			...
			v = [ll[0],ll[3],ll[4],ll[6],start+stop,{}] + [0]*2*len(species) # five columns with general info.
		if v[6 + s] > 0:
			v[5][s] = s
		if s != sp:
			v[6 + s] = max(v[6 + s],float(ll[5])) #max score (across species)
		else:
			v[6 + s] = 1
		v[6 + len(species) + s] = max(v[6 + len(species) + s],int(ll[8].split(":")[6])) #max number of samples where intron was found (across species)
		res[k] = v
	gtf.close()
	return(res)
...
def printMergedResults(t):
	for k in t:
		v = t[k]
		v[5] = ''.join(sorted([str(k) for k in v[5]])) #6th column contains indexes of all species where intron was found
