...
# conservation of devAS vs non-devAS
...
# use only exons with adult PSI < 0.9 in given tissue
...
pdf('figures/paper.figures/3.2/1.pdf',w=9,h=12)
#layout(matrix(c(3,1,3,2),ncol=2))
layout(matrix(c(1,1,2:11),byrow = TRUE,ncol=3))
...
#barplot(filtered.seg.cnt[1:3,],den=den,ylab='Number of exons',las=3,main='Cassette exons passed filtering',legend.text=c('cod.','partially cod.','non cod.'))#,col=col)
...
plotPanelLetter('E')
#plotSpCor2Cor(mr.cors$ndev,mr.cors$dev,age.al.i[,c('mouse','rat')],meta,main='Mouse-rat Spearman correlation',ylab='devAS (2243)',xlab='not devAS (3112)',same.lims=F)

dev2ndev.1f = calcDevVsNdevCor('mouse','rat',1,F)
plot(dev2ndev.1f$ndevAS.cor,dev2ndev.1f$devAS.cor,col=meta.tsm[dev2ndev.1f$sid,'col'],cex=meta.tsm[dev2ndev.1f$sid,'cex']*3,pch=19,xlab='mouse-rat, Spearman, not devAS',ylab='mouse-rat, Spearman, devAS',main='aPSI <= 1; devAS in at least 1 tis.')
abline(a=0,b=1)
plotPanelLetter('F1')

dev2ndev.1f = calcDevVsNdevCor('mouse','rat',0.9,F)
plot(dev2ndev.1f$ndevAS.cor,dev2ndev.1f$devAS.cor,col=meta.tsm[dev2ndev.1f$sid,'col'],cex=meta.tsm[dev2ndev.1f$sid,'cex']*3,pch=19,xlab='mouse-rat, Spearman, not devAS',ylab='mouse-rat, Spearman, devAS',main='aPSI <= 0.9 both; devAS in at least 1 tis.')
abline(a=0,b=1)
plotPanelLetter('F2')

dev2ndev.1f = calcDevVsNdevCor('mouse','rat',0.9,T)
plot(dev2ndev.1f$ndevAS.cor,dev2ndev.1f$devAS.cor,col=meta.tsm[dev2ndev.1f$sid,'col'],cex=meta.tsm[dev2ndev.1f$sid,'cex']*3,pch=19,xlab='mouse-rat, Spearman, not devAS',ylab='mouse-rat, Spearman, devAS',main='aPSI <= 0.9 both; devAS in given tis.')
abline(a=0,b=1)
plotPanelLetter('F3')


# dev2ndev.1f = calcDevVsNdevCor('mouse','rat',1,F,psi.in.both = F)
# plot(dev2ndev.1f$ndevAS.cor,dev2ndev.1f$devAS.cor,col=meta.tsm[dev2ndev.1f$sid,'col'],cex=meta.tsm[dev2ndev.1f$sid,'cex']*3,pch=19,xlab='mouse-rat, Spearman, not devAS',ylab='mouse-rat, Spearman, devAS',main='aPSI <= 1; devAS in at least 1 tis.')
# abline(a=0,b=1)
# plotPanelLetter('F4')
plot.new()
dev2ndev.1f = calcDevVsNdevCor('mouse','rat',0.9,F,psi.in.both = F)
plot(dev2ndev.1f$ndevAS.cor,dev2ndev.1f$devAS.cor,col=meta.tsm[dev2ndev.1f$sid,'col'],cex=meta.tsm[dev2ndev.1f$sid,'cex']*3,pch=19,xlab='mouse-rat, Spearman, not devAS',ylab='mouse-rat, Spearman, devAS',main='aPSI <= 0.9 a.l.o.; devAS in at least 1 tis.')
abline(a=0,b=1)
plotPanelLetter('F4')

dev2ndev.1f = calcDevVsNdevCor('mouse','rat',0.9,T,psi.in.both = F)
plot(dev2ndev.1f$ndevAS.cor,dev2ndev.1f$devAS.cor,col=meta.tsm[dev2ndev.1f$sid,'col'],cex=meta.tsm[dev2ndev.1f$sid,'cex']*3,pch=19,xlab='mouse-rat, Spearman, not devAS',ylab='mouse-rat, Spearman, devAS',main='aPSI <= 0.9 a.l.o.; devAS in given tis.')
abline(a=0,b=1)
plotPanelLetter('F5')
dev.off()
...
mouse.ad.tsm.mds = readRDS('Rdata/paper.figures/mouse.ad.tsm.mds.Rdata')
# calc divergence to embrio
#mouse.ge = readRDS('Rdata/ens.ge.cod.tsm.Rdata')$mouse
mouse.ge = readRDS('Rdata/ens.ge.marg.tsm.Rdata')$mouse
#mouse.ge = ens.ge.marg.tsm$mouse
t = read.csv('input/gene.info.from.marg/MouseDDGs.Biotype.csv')
mouse.ge = mouse.ge[rownames(mouse.ge) %in% t$Mouse_ID[t$Biotype=='protein_coding'],]
rm(t)
gc()
...
psi.cor2embryo = caclCor2Embryo(psi.tsm$mouse[anns$mouse$sites=='ad',],meta.tsm,cor.m = 'p',use.mean.embryo=use.mean.embryo)
...
#sf.cor2embryo = caclCor2Embryo(mouse.ge[rownames(mouse.ge) %in% unlist(mouse.go.rev[c('GO:0003723','GO:0008380')]),],meta.tsm,cor.m = 'p',use.mean.embryo=use.mean.embryo)
sf.cor2embryo = caclCor2Embryo(mouse.ge[rownames(mouse.ge) %in% cisbp$DBID[cisbp$RBP_Species=='Mus_musculus'],],meta.tsm,cor.m = 'p',use.mean.embryo=use.mean.embryo)
...
pdf('figures/paper.figures/3.2/2.pdf',w=9,h=12)
...
tissues = unique(meta$tissue)
m.sp.cor = lapply(tissues,function(t)calcCor2TisOnDev(psi.tsm$mouse[anns$mouse$sites=='ad',],t,meta.tsm,cor.meth = 'sp'))
m.pe.cor = lapply(tissues,function(t)calcCor2TisOnDev(psi.tsm$mouse[anns$mouse$sites=='ad',],t,meta.tsm,cor.meth = 'pe'))
h.sp.cor = lapply(tissues,function(t)calcCor2TisOnDev(psi.tsm$human[anns$human$sites=='ad',],t,meta.tsm,cor.meth = 'sp'))
h.pe.cor = lapply(tissues,function(t)calcCor2TisOnDev(psi.tsm$human[anns$human$sites=='ad',],t,meta.tsm,cor.meth = 'pe'))

names(h.sp.cor) = names(h.pe.cor) = names(m.sp.cor) = names(m.pe.cor) = tissues

pdf('figures/paper.figures/3.2/2.tissue-tissue.cor.on.dev.pdf',w=9,h=9)
par(mfrow=c(3,3),tck=-0.02,mgp=c(1.1,0.2,0),mar=c(2.5,2,1.5,0),oma=c(0,0,1,1))
for(t in tissues)
	plotTissueAgeProile(m.sp.cor[[t]],meta.tsm,age.axis='rank',main=t,ylab=paste0('Spearman correlation to ',t),xlim=range(meta.tsm$age.rank[meta.tsm$species=='mouse']))
mtext('Mouse, Spearman',outer=T)
par(mfrow=c(3,3),tck=-0.02,mgp=c(1.1,0.2,0),mar=c(2.5,2,1.5,0),oma=c(0,0,1,1))
for(t in tissues)
	plotTissueAgeProile(m.pe.cor[[t]],meta.tsm,age.axis='rank',main=t,ylab=paste0('Pearson correlation to ',t),xlim=range(meta.tsm$age.rank[meta.tsm$species=='mouse']))
mtext('Mouse, Pearson',outer=T)

par(mfrow=c(3,3),tck=-0.02,mgp=c(1.1,0.2,0),mar=c(2.5,2,1.5,0),oma=c(0,0,1,1))
for(t in tissues)
	plotTissueAgeProile(h.sp.cor[[t]],meta.tsm,age.axis='rank',main=t,ylab=paste0('Spearman correlation to ',t),xlim=range(meta.tsm$age.rank[meta.tsm$species=='mouse']))
mtext('Human, Spearman',outer=T)
par(mfrow=c(3,3),tck=-0.02,mgp=c(1.1,0.2,0),mar=c(2.5,2,1.5,0),oma=c(0,0,1,1))
for(t in tissues)
	plotTissueAgeProile(h.pe.cor[[t]],meta.tsm,age.axis='rank',main=t,ylab=paste0('Pearson correlation to ',t),xlim=range(meta.tsm$age.rank[meta.tsm$species=='mouse']))
mtext('Human, Pearson',outer=T)
dev.off()
### figure 3 prepare #####
#A
...
### figure 3 plot #####
pdf('figures/paper.figures/3.2/3.pdf',w=10,h=11)
s = 'mouse'
...
text(b[21], log(500),'inclusion',adj=c(-0.1,1.1))
text(b[21],-log(500),'exclusion',adj=c(-0.1,-1.1))
plotPanelLetter('B')
...
# plot
pdf('figures/paper.figures/3.2/4.pdf',w=10,h=13)
...
ens.ge.cod.tsm.log = lapply(names(ens.ge.marg.tsm),function(s)log(ens.ge.marg.tsm[[s]][orth[f,s],]+1))
...
ge.mr.cor=calcBootstrapSpeciesDiv(ens.ge.cod.tsm.log,c('mouse','rat'),function(x){x['mouse','rat']},age.al.i,0)
ge.mh.cor=calcBootstrapSpeciesDiv(ens.ge.cod.tsm.log,c('mouse','human'),function(x){x['mouse','human']},age.al.i[-5,],0)
as.mr.cor=calcBootstrapSpeciesDiv(orth.seg.ad.tsm,c('mouse','rat'),function(x){x['mouse','rat']},age.al.i,0,qv=orth.per.tissue.age.qv)
as.mh.cor=calcBootstrapSpeciesDiv(orth.seg.ad.tsm,c('mouse','human'),function(x){x['mouse','human']},age.al.i[-5,],0,qv=orth.per.tissue.age.qv)


# par(mfrow=c(2,2),tck=-0.02,mgp=c(1.1,0.2,0),mar=c(3,2,1.5,0),oma=c(0,0,2,1))
# plotDivergenceOnAgeWithConf(ge.mr.cor,params$tissue.col,ylab='Total tree length (1-cor)',main='GE divergence (mr)')
# plotDivergenceOnAgeWithConf(ge.mh.cor,params$tissue.col,ylab='Total tree length (1-cor)',main='GE divergence (mh)')
# plotDivergenceOnAgeWithConf(as.mr.cor,params$tissue.col,ylab='Total tree length (1-cor)',main='AS divergence (mr)')
# plotDivergenceOnAgeWithConf(as.mh.cor,params$tissue.col,ylab='Total tree length (1-cor)',main='AS divergence (mh)')
...
pdf('figures/paper.figures/3.2/5.devAS.pdf',w=10,h=8)
...
### 6 Direction new/alternification ####
# new exons
...
par(mfrow=c(2,3),tck=-0.02,mgp=c(2,0.2,0),mar=c(3.5,3,1.5,0),oma=c(0,0,0,1))
...
pdf('figures/paper.figures/3.2/6.pdf',w=9,h=9)
...
## 6G for mouse and per tissue ####
mdevD = orth.per.tissue.age.qv$mouse<0.05 & orth.seg.ad.all.dpsi$mouse[rownames(orth.per.tissue.age.qv$mouse),] < -0.2
mdevU = orth.per.tissue.age.qv$mouse<0.05 & orth.seg.ad.all.dpsi$mouse[rownames(orth.per.tissue.age.qv$mouse),] >  0.2
f = is.na(orth.per.tissue.age.qv$mouse) | is.na(orth.seg.ad.all.dpsi$mouse[rownames(orth.per.tissue.age.qv$mouse),])
mdevD[f] = mdevU[f] = NA

alt.ids=apply(sapply(orth.seg.ad,function(x)x$seg$alt.id),1,paste,collapse=' ')
alt.sp = readRDS('Rdata/paper.figures/alt.sp.Rdata')
#take just one exon from complex alternatives..
# set.seed(3781)
# alt.filt = 1:length(alt.ids) %in% sapply(split(1:length(alt.ids),alt.ids),function(x){if(length(x)==1){x}else{sample(x,size=1)}})

sps = c('m','mr','mrb','hqmrb','hqmrbo','hqmrboc')
f = T
malt.devD.prop.per.tissue = lapply(unique(meta$tissue),function(t)getSgnFraqWithBootstrap(alt.sp[alt.sp!=''][f],sps,matrix(mdevD[f,t],ncol=1,dimnames = list(NULL,'mouse')),N=500))
malt.devU.prop.per.tissue = lapply(unique(meta$tissue),function(t)getSgnFraqWithBootstrap(alt.sp[alt.sp!=''][f],sps,matrix(mdevU[f,t],ncol=1,dimnames = list(NULL,'mouse')),N=500))
malt.devS.prop.per.tissue = lapply(unique(meta$tissue),function(t)getSgnFraqWithBootstrap(alt.sp[alt.sp!=''][f],sps,matrix(mdevU[f,t] | mdevD[f,t],ncol=1,dimnames = list(NULL,'mouse')),N=500))
names(malt.devS.prop.per.tissue) = names(malt.devU.prop.per.tissue) = names(malt.devD.prop.per.tissue) = unique(meta$tissue)
# 
# f=alt.filt
# malt.devD.prop.per.tissue.a = lapply(unique(meta$tissue),function(t)getSgnFraqWithBootstrap(alt.sp[alt.sp!=''][f],sps,matrix(mdevD[f,t],ncol=1,dimnames = list(NULL,'mouse')),N=500))
# malt.devU.prop.per.tissue.a = lapply(unique(meta$tissue),function(t)getSgnFraqWithBootstrap(alt.sp[alt.sp!=''][f],sps,matrix(mdevU[f,t],ncol=1,dimnames = list(NULL,'mouse')),N=500))
# malt.devS.prop.per.tissue.a = lapply(unique(meta$tissue),function(t)getSgnFraqWithBootstrap(alt.sp[alt.sp!=''][f],sps,matrix(mdevU[f,t] | mdevD[f,t],ncol=1,dimnames = list(NULL,'mouse')),N=500))
# names(malt.devS.prop.per.tissue.a) = names(malt.devU.prop.per.tissue.a) = names(malt.devD.prop.per.tissue.a) = unique(meta$tissue)

sapply(split(mdevU[,'brain'],alt.sp[alt.sp!=''])[sps],table)

maxDir = orth.seg.ad.all.dpsi$mouse[rownames(orth.per.tissue.age.qv$mouse),]
maxDir[is.na(orth.per.tissue.age.qv$mouse)] = NA
maxDir[orth.per.tissue.age.qv$mouse>0.05 | abs(maxDir) < 0.2] = 0
maxDir = apply(maxDir,1,function(x){x = x[!is.na(x)];if(length(x)==0){return(NA)};sign(x[which.max(abs(x))])})

getSgnFraqWithBootstrap(alt.sp[alt.sp!=''],sps,matrix(maxDir==-1,ncol=1,dimnames = list(NULL,'mouse')),N=100)
sapply(split(maxDir[alt.filt],alt.sp[alt.sp!=''][alt.filt])[sps],table)

pdf('figures/paper.figures/3.2/2018.07.18/6G.per.tissue.pdf',w=9,h=9)
par(mfrow=c(3,3),tck=-0.02,mgp=c(1.3,0.3,0),mar=c(5,3,1.5,0),oma=c(0,0,0,1))
for(t in names(malt.devD.prop.per.tissue)){
	plotArea(1:6,malt.devS.prop.per.tissue[[t]][,-2],new = T,col=params$tissue.col[t],lwd=6,xlab='',xaxt='n',ylab='devAS fraction',main=t)
	axis(1,1:6,rownames(malt.devS.prop.per.tissue[[t]]),las=3)
}

par(mfrow=c(3,3),tck=-0.02,mgp=c(2,0.2,0),mar=c(3.5,3,1.5,0),oma=c(0,0,0,1))
for(t in names(malt.devD.prop.per.tissue)){
	plotArea(1:6,malt.devD.prop.per.tissue[[t]][,-2],new = T,col=params$tissue.col[t],lwd=6,xlab='',area.den = 40,xaxt='n',ylab='devAS fraction',main=t,ylim=range(0,malt.devU.prop.per.tissue[[t]][,-2],malt.devD.prop.per.tissue[[t]][,-2]))
	plotArea(1:6,malt.devU.prop.per.tissue[[t]][,-2],new = F,col=params$tissue.col[t],lwd=6,lty=2,area.den = -1)
	axis(1,1:6,rownames(malt.devS.prop.per.tissue[[t]]),las=3)
}
plot.new()
legend('topleft',den=c(-1,40),fill='black',legend=c('inclusion','exclusion'))
dev.off()

ids = which(alt.filt & !is.na(mdevD[,1]) & mdevD[,1] & alt.sp[alt.sp!='']=='m')
ids = which(!is.na(maxDir) & maxDir==1& alt.sp[alt.sp!='']=='m') 
length(ids)
s='mouse'
par(mfrow=c(4,4),tck=-0.02,mgp=c(2,0.2,0),mar=c(3.5,3,1.5,0),oma=c(0,0,0,1))
for(s in rownames(species))
for(i in ids)
	plotTissueAgeProile(orth.seg.ad[[s]]$ir[i,],meta,main=s,ylim=c(0,1))
